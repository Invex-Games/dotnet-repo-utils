...
public interface INugetPackageUnlistHelper : IReportsHelper
{
    ...
    static IReadOnlyList<SemVer> SelectSupersededPrereleases(
        ...
        publishedVersions
            .Where(version =>
                version.IsPreRelease && version.Prefix == currentVersion.Prefix && version < currentVersion)
    ...
    /// </summary>
    /// <param name="version">
    ///     The exclusive version ceiling (typically a stable version); only prereleases with lower
    ///     precedence are selected.
    /// </param>
    /// <param name="publishedVersions">All versions currently published for the package.</param>
    /// <returns>
    ...
    /// </summary>
    /// <param name="feedUrl">
    ///     The service index URL of the NuGet feed (for example, <c>https://api.nuget.org/v3/index.json</c>
    ///     ).
    /// </param>
    ...
    Task<IReadOnlyList<UnlistResult>> UnlistSupersededPrereleasesForPackages(
        ...
        UnlistPrereleasesForPackages(feedUrl,
            apiKey,
            packageIds,
        ...
        CancellationToken cancellationToken)
    {
        ...
        var flatContainerBase =
            await GetServiceResourceAsync(client, feedUrl, FlatContainerResourceType, cancellationToken);
        ...
        foreach (var packageId in packageIds)
        {
            var publishedVersions =
                await GetPublishedVersionsAsync(client, flatContainerBase, packageId, cancellationToken);

            var toUnlist = selectVersions(publishedVersions);

            Logger.LogInformation("Found {Count} prerelease version(s) of {PackageId} to unlist.",
                toUnlist.Count,
                packageId);

            foreach (var version in toUnlist)
                results.Add(
                    await UnlistPackageAsync(client, publishBase, apiKey, packageId, version, cancellationToken));
        }
    ...
    /// <param name="cancellationToken">A token used to cancel the operation.</param>
    /// <returns>
    ///     The resource base address without a trailing slash, or <see langword="null" /> when the resource is not
    ///     present.
    /// </returns>
    ...
    {
        using var response =
            await SendWithRetryAsync(() => client.GetAsync(feedUrl, cancellationToken), cancellationToken);

        if (!response.IsSuccessStatusCode)
        {
            Logger.LogWarning("Could not read service index from {Feed}: HTTP {StatusCode}.",
                feedUrl,
    ...
    private static bool ResourceTypeMatches(JsonElement typeProperty, string resourceType)
    {
        ...
        {
            return type is not null && type.StartsWith(resourceType, StringComparison.OrdinalIgnoreCase);
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!response.IsSuccessStatusCode)
        {
            Logger.LogWarning("Could not read versions for {PackageId}: HTTP {StatusCode}.",
                packageId,
                (int)response.StatusCode);
        ...
        CancellationToken cancellationToken)
    {
        for (var attempt = 1;; attempt++)
            try
            {
                ...
                Logger.LogWarning("Transient HTTP {StatusCode} on attempt {Attempt}/{MaxAttempts}; retrying.",
                    (int)response.StatusCode,
                    attempt,
                    MaxHttpAttempts);
            ...
            {
                Logger.LogWarning("HTTP request timed out on attempt {Attempt}/{MaxAttempts}; retrying.",
                    attempt,
                    MaxHttpAttempts);
    ...
    private void AddUnlistReport(string reportTitle, string emptyReportMessage, IReadOnlyList<UnlistResult> results)
    {
        ...
        var rows = results
            .Select(result => (IReadOnlyList<string>)
            [
                result.PackageId,
